...
func runSetupFlow() {
	...
	fmt.Println(dimStyle.Render("   Run: vibescan config set anthropic-api-key <your-key>"))
	fmt.Println()
	fmt.Println(optStyle.Render("2. Set up MCP for Claude Code"))
	fmt.Println(dimStyle.Render("   Run: claude mcp add armur -- vibescan mcp"))
	fmt.Println()
	fmt.Println(optStyle.Render("3. Set up MCP for Cursor"))
	fmt.Println(dimStyle.Render("   Run: vibescan mcp setup cursor"))
	fmt.Println()
	fmt.Println(optStyle.Render("4. Initialize project config"))
	fmt.Println(dimStyle.Render("   Run: vibescan init"))
	fmt.Println()
}
